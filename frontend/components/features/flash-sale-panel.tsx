  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
      <Card className="lg:col-span-7">
        <CardHeader>
          <div>
            <CardTitle>{t("Hành vi trên trang sản phẩm")}</CardTitle>
        ...
        <CardContent className="space-y-4">
          <div>
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium text-text-dim">{t("Thời gian dừng trên trang (giây)")}</label>
          ...
          <div>
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium text-text-dim">{t("Độ sâu scroll (%)")}</label>
          ...
          <div>
            <div className="flex items-center justify-between">
              <label className="text-xs font-medium text-text-dim">{t("Số lần quay lại xem")}</label>
          ...
          <label className="flex items-center gap-2 text-xs text-text-muted">
            ...
            {t("Đã thêm vào giỏ nhưng chưa mua")}
          </label>
      ...
      <Card className="lg:col-span-5">
        ...
        <CardContent>
          {error ? (
            ...
            <p className="text-sm text-text-muted">{t("Bấm Phân tích để xem đề xuất.")}</p>
          ) : (
            <div className="space-y-4">
              ...
              <div className="mono text-xs text-text-muted">{tf("Điểm do dự: {điểm}", { điểm: result.hesitation_score })}</div>
              {result.suggested_discount_pct > 0 && (
                <div className="rounded-md border border-border bg-bg-alt px-3 py-2 text-xs text-text">
                  {t("Giảm giá đề xuất:")} <span className="mono font-semibold text-accent">{result.suggested_discount_pct}%</span>
                </div>
              )}
